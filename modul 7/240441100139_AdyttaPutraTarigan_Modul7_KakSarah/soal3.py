def tambah_siswa(siswa):
    ...
    for siswa_data in siswa:
        if siswa_data["nama"].lower() == nama.lower():
            print(f"Siswa dengan nama '{nama}' sudah ada. Silakan gunakan nama lain.\n")
            return
...
def tampilkan_siswa(siswa):
    ...
    nomor = 1
    for siswa_data in siswa:
        print(f"{nomor}. Nama: {siswa_data['nama']}, Kelas: {siswa_data['kelas']}, Sekolah: {siswa_data['sekolah']}, Plotting: {siswa_data['plotting']}")
        nomor += 1
...
def ubah_siswa(siswa):
    ...
    index = int(input("Nomor siswa yang ingin diubah: ")) - 1
    if 0 <= index < len(siswa):
        siswa_data = siswa[index]
        ...
        siswa_data['sekolah'] = input(f"Asal Sekolah ({siswa_data['sekolah']}): ") or siswa_data['sekolah']
...
def hapus_siswa(siswa):
    ...
    tampilkan_siswa(siswa)
    index = int(input("Nomor siswa yang ingin dihapus: ")) - 1
    if 0 <= index < len(siswa):
        siswa_dihapus = siswa.pop(index)
